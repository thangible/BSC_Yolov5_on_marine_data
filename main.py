...
os.system('python3 train.py --data dataset.yaml --hyp custom_hyp.yaml --weights yolov5s.pt --cache --epochs 100 --run_name {}'.format('Flipping'))
time.sleep(60)
create_augmented_images(augmentation = Rotation, olddir = OLDDIR, newdir = NEWDIR, maskdir = MASKDIR, data_info_path = DATA_INFO, is_normalize = False)
os.system('python3 train.py --data dataset.yaml --hyp custom_hyp.yaml --weights yolov5s.pt --cache --epochs 100 --run_name {}'.format('Rotation'))
time.sleep(60)
# create_augmented_images(augmentation = Solarize, olddir = OLDDIR, newdir = NEWDIR, maskdir = MASKDIR, data_info_path = DATA_INFO, is_normalize = False)
...
# time.sleep(60)
# create_augmented_images(augmentation = MotionBlur, olddir = OLDDIR, newdir = NEWDIR, maskdir = MASKDIR, data_info_path = DATA_INFO, is_normalize = False)
